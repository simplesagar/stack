...
func NewListPaymentsCommand() *cobra.Command {
	return fctl.NewCommand("list",
		...
		fctl.WithRunE(func(cmd *cobra.Command, args []string) error {
			...
			paymentsCursor, err := client.Payments.ListPayments(cmd.Context(), request)
			if err != nil {
				return fctl.WrapError(err, "listing payments")
			}

			tableData := fctl.Map(paymentsCursor.PaymentsCursor.Cursor.Data, func(payment shared.Payment) []string {
				return []string{
					payment.ID,
					string(payment.Type),
